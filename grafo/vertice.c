...
void vertice_visitado(vertice_t *vertice, int vist){
    ...
    if (vertice == NULL){
        fprintf(stderr, "vertice_visitado: vertice invalido\n");
        exit(EXIT_FAILURE);
    }

    vertice->visitado=vist;

}

int vertice_get_visit(vertice_t *vertice){

    if (vertice == NULL){
        fprintf(stderr, "vertice_get_visit: vertice invalido\n");
        exit(EXIT_FAILURE);
    }
